...
class Validator implements ValidatorContract
{
    ...
    protected $implicitRules = [
        'Required', 'Filled', 'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredWithWithout', 'RequiredIfWithWithout', 'RequiredWithoutAll',
        'RequiredIf', 'RequiredIfWithout', 'RequiredIfSet', 'RequiredUnless', 'Accepted', 'Present', 'DateFormat', 'RequiredIfBankVerified', 'BankEditable'
    ...
    protected $dependentRules = [
        'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredIfWithWithout', 'RequiredWithoutAll', 'RequiredWithWithout',
        'RequiredIf', 'RequiredIfSet', 'RequiredUnless', 'Confirmed', 'Same', 'Different', 'Unique',
        'Before', 'After'
    ];
    protected $DBRules = [
        'DbMin', 'DbMax', 'DbRegex', 'DbIn', 'DbDigits', 'DbInArray', 'DbAfter', 'DbBefore', 'DbMimes', 'DbNotIn', 'DbSize', 'DbBetween', 'DbDigitsBetween', 'DbMimetypes'
    ];
    ...
    protected function validate ($attribute, $rule)
    {
        ...
        if (in_array($rule, $this->DBRules) && strstr($rule, 'Db'))
        {
            if (isset($this->failedRules[str_replace('required_with:', '', $this->rules[$attribute][0])]))
            {
                return;
            }
            $parameters = [$this->getDBValue($attribute, $rule, $parameters)];
            $rule = str_replace('Db', '', $rule);
        }
    ...
    public function getDBValue ($attribute, $rule, $parameters)
    {
        $this->requireParameterCount(2, $parameters, $rule);
        $valueColumn = $parameters[0];
        list($connection, $table) = $this->parseTable($parameters[1]);
        $extra = $this->getExtraExistConditions($parameters, 2);
        if (isset($parameters[3]))
        {
            $inputValue = null;
            if (strstr($this->rules[$attribute][0], 'required_with:'))
            {
                $checkValue = Arr::get($this->data, $parameters[3]);
                if (is_null($checkValue))
                {
                    throw new InvalidArgumentException("Validation rule $rule DB value is null.");
                }
            }
            else
            {
                $checkValue = $parameters[3];
            }
            $extra = [$parameters[2]=>$checkValue];
        }
        $verifier = $this->getPresenceVerifier();
        $verifier->setConnection($connection);
        return $verifier->getValue($table, $valueColumn, null, null, $extra);
    }
    ...
    {
        return in_array($rule, ['Unique', 'Exists', 'Before']) ? !$this->messages->has($attribute) : true;
    }
     ...
     * @return bool
     */
    protected function validateRequiredWithWithout ($attribute, $value, $parameters)
    {
        if (!$this->allFailingRequired(explode('~', $parameters[0])) && $this->anyFailingRequired(explode('~', $parameters[1])))
        {
            return $this->validateRequired($attribute, $value);
        }
        return true;
    ...
    protected function validateRequiredIfWithout ($attribute, $value, $parameters)
    {
        $this->requireParameterCount(2, $parameters, 'required_if_without');
        $data = Arr::get($this->data, $parameters[0]);
        $values = array_slice($parameters, 1);
        if (is_bool($data))
        {
        ...
        if (in_array($data, $values) && $this->anyFailingRequired([$parameters[2]]))
        {
            return $this->validateRequired($attribute, $value);
        }
        return true;
     ...
     * @return bool
     */
    protected function validateRequiredIfSet ($attribute, $value, $parameters)
    {
        $this->requireParameterCount(2, $parameters, 'required_if_set');
        $required = true;
        array_walk($parameters, function($parameter) use(&$required)
        {
            $parameter = explode('~', $parameter);
            $data = Arr::get($this->data, $parameter[0]);
            $value = $parameter[1];
            if ($value === 'true')
            {
                $value = true;
     ...
     * @return bool
     */
    protected function validateRequiredIfSets ($attribute, $value, $parameters)
    {
        $this->requireParameterCount(2, $parameters, 'required_if_sets');
        $required = true;
        array_walk($parameters, function($parameter) use(&$required)
        {
            $parameter = explode('~', $parameter);
    ...
    protected function validateRequiredIfWithWithout ($attribute, $value, $parameters)
    {
        $this->requireParameterCount(4, $parameters, 'required_if_with_without');
        $data = Arr::get($this->data, $parameters[0]);
        $check = $parameters[1];
        if (is_bool($data))
        {
        ...
        if ($data == $check && !$this->allFailingRequired([$parameters[2]]) && $this->anyFailingRequired([$parameters[3]]))
        {
            return $this->validateRequired($attribute, $value);
        }
        return true;
     ...
     * @return array
     */
    protected function getExtraExistConditions (array $parameters, $remove = 2)
    {
        return $this->getExtraConditions(array_values(array_slice($parameters, $remove)));
    }
    ...
    {
        array_walk($parameters, function(&$p) use($message)
        {
            $p = str_replace(['_', '='], [' ', $message['=']], $p);
        });
        return str_replace([':params'], implode($message['concat'], $parameters), $message['msg']);
    }
    ...
    protected function replaceRequiredWithWithout ($message, $attribute, $rule, $parameters)
    {
        $parameters = $this->getAttributeList($parameters);
        return str_replace([':with', ':wout'], ['with'=>$parameters[0], 'with_out'=>$parameters[1]], $message);
    }
    ...
    protected function replaceRequiredIfSet ($message, $attribute, $rule, $parameters)
    {
        $sets = [];
        array_walk($parameters, function($parameter) use(&$sets, $message)
        {
            $parameter = explode('~', $parameter);
            $parameter[1] = $this->getDisplayableValue($parameter[0], Arr::get($this->data, $parameter[0]));
            $parameter[0] = $this->getAttribute($parameter[0]);
            $sets[] = str_replace([':other', ':value'], $parameter, $message['sets']);
        });
        $sets = [implode($message['concat'], $sets)];
        return str_replace([ ':sets'], $sets, $message['msg']);
    }
    ...
    protected function replaceRequiredIfWithWithout ($message, $attribute, $rule, $parameters)
    {
        $parameters[1] = $this->getDisplayableValue($parameters[0], Arr::get($this->data, $parameters[0]));
        $parameters[2] = $this->getAttribute($parameters[2]);
        $parameters[3] = $this->getAttribute($parameters[3]);
        $parameters[0] = $this->getAttribute($parameters[0]);

        return str_replace([':other', ':value', ':with', ':wout'], $parameters, $message);
    }
     ...
     */
    public function messages ($formatToArray = false)
    {
        if (!$this->messages)
        {
            $this->passes();
        }
        return $formatToArray ? $this->dotToArrayName($this->messages) : $this->messages;
    }

    public function dotToArrayName ($array)
    {
        $array = json_decode($array, true);
        $formatted_array = [];
        if (is_array($array))
        {
            array_walk($array, function($str, $k) use(&$formatted_array)
            {
    ...
    protected function validateRequiredIfBankVerified ($attribute, $value, $parameters)
    {
        $this->requireParameterCount(2, $parameters, 'required_if_bank_verified');
        $data = Arr::get($this->data, $parameters[0]);
        $values = array_slice($parameters, 1);
        if (is_bool($data))
        {
    ...
    protected function validateBankEditable ($attribute, $value, $parameters)
    {
        $this->requireParameterCount(2, $parameters, 'bank_editable');
        $data = Arr::get($this->data, $parameters[0]);
        $values = array_slice($parameters, 1);
        if (is_bool($data))
        {
