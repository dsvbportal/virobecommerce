...
if (!function_exists('getGTZ'))
{

    /**
     * Format a Global time/date
     * $date is null means return current time in global timezone
     * $date is not null return local time to global timezone for DB
     *
     * @param string $date [optional] <p>Global Time String Format.</p>
     * @param string $format [optional] <p>The format of the outputted date string</p>
     * @return string a formatted date string.
     */
    function getGTZ ($date = null, $format = 'Y-m-d H:i:s')
    {
        if (!empty($date))
        {
            ...
            elseif (preg_match('/^\d{1,4}[-\/]\d{1,4}[-\/]\d{1,4}$/', $date))
            {
                $date = date_format(date_time_set(date_create($date), date('H'), date('i'), date('s')), 'Y-m-d H:i:s');
            }
        }
        return !empty($date) ? gmdate($format, strtotime($date)) : gmdate($format);
    }

}
if (!function_exists('showUTZ'))
{

    /**
     * Format a local time/date
     * $date is null means return current time in local specified or d-M-Y H:i:s format
     * $date is not null return db global time to local time
     *
     * @param string $date [optional] <p>Global Time String Format.</p>
     * @param string $format [optional] <p>The format of the outputted date string</p>
     * @return string a formatted date string.
     */
    function showUTZ ($date = null, $format = 'd-M-Y H:i:s')
    {
        
		
		if (!empty($date) && preg_match('~[a-zA-Z]~', $date))
        {
            $format = $date;
            $date = null;
        }
        ...
        return $date ? date($format, strtotime((new \DateTime($date, new \DateTimeZone('+00:00')))->format('c'))) : date($format);
    }
...
if (!function_exists('Generate_Reg_Token'))
{
    /**
     * Show partially hidden mobile numbers      
     * @return string a formatted mobile numbers string.
     */
    function Generate_Reg_Token($token)
    {
        if ($token)
        {
            ...
            //return route('seller.verifyEmailLink', ['token' => $token]);
            return $token;
        }
    }
}
	
if (!function_exists('Decrypt_Reg_Token'))
{
    /**
     * Show partially hidden mobile numbers      
     * @return string a formatted mobile numbers string.
     */	
	function Decrypt_Reg_Token($token)
    {
        if ($token)
        {
            $char_length = $token[0];
