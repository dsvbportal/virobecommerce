...
class BonusController extends AffBaseController {
	...
	/*
	 * Credit Personal bonus commission
	 *   
	*/
	
	public function credit_pcc_bonus(){
		$op['status'] = $this->statusCode = 422;
		$op['msg'] = 'error';
		$postdata = $this->request->all();
		$res = $this->bonusObj->add_bonus($postdata);
	...
	public function package_purchase_bonus(){
		$postdata['account_id'] 	= 8;
		$postdata['amount'] 		= 800;
		$postdata['relation_id'] 	= 10;
		$result 					= $this->bonusObj->package_purchase_bonus_commission($postdata);
		print_r($result);exit;
	}
