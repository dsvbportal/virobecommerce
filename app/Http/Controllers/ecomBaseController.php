...
class ecomBaseController extends Controller
{
    ...
    {		
        ...
        $this->device_log_id =  $this->config->get('device_log')->device_log_id;
        $this->token =  $this->config->get('device_log')->token;
        ...
        $this->pagesettings->account_type = $this->config->get('constants.ACCOUNT_TYPE.USER');  //$this->config->get('account_type'); 
        $this->country_id = 77;
        $this->state_id = null;
        $this->region_id = null;
        $this->city_id = null;
        ...
        {	
            //$this->config->set('data.user',$this->session->get('userdata'));	
            $this->config->set('app.accountInfo',$this->session->get('userdata'));	
            ...
            {		
                ...
                {	
                    ...
                    View::share('device_log', $this->config->get('device_log')); 
					//View::share('userSess', $this->userSess);
                }								
            }
        }
		 $validiate['lfields'] = CommonNotifSettings::getHTMLValidation('ecom.checklogin'); 
         $validiate['regfields'] = CommonNotifSettings::getHTMLValidation('ecom.sign_up_save');  
         $validiate['ip_country'] = $this->commonObj->getIpCountry('192.168.1.15');                                    
         $validiate['countries']= $this->lcObj->getCountries(['operate'=>true]);    
         foreach ($validiate['countries'] as $key => $value) {
                    $validiate['countries'][$key]->img_url=asset('resources/assets/imgs/flags')."/".mb_strtolower($value->iso2).".png";
         }
		View::share('validiate', $validiate);	
