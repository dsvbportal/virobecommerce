...
class DataController extends ecomBaseController
{
    ...
    {		   
        ...
        {
            case 'home':
                $op['sliders'] = guzzle::getResponse(config('services.api.url').'get-sliders', 'POST', [], ['page'=>'home']);
				...
				$res['header_catalogue'] = guzzle::getResponse(config('services.api.url').'main-categories', 'POST', [], []);
                $op['menus'] = guzzle::getResponse(config('services.api.url').'get-menus', 'POST', [], []);
				...
				if(isset($op['menus']->menu->header_primary) && (!empty($op['menus']->menu->header_primary))){
					array_walk($op['menus']->menu->header_primary,function(&$menu){
               ...
               /*  $op['my_cart'] = ShoppingPortal::getResponse('api/v1/customer/products/my-cart-count', 'POST', []); */
                $res['main_categories'] = guzzle::getResponse(config('services.api.url').'main-categories', 'POST', [], []);
				array_walk($res['main_categories']->category, function(&$category)
				{
					$category->url = $this->commonObj->generateUrl($category);					
				});
				$op['main_categories'] = $res['main_categories'];
