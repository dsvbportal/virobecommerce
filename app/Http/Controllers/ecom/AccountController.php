...
class AccountController extends ecomBaseController
{
    ...
    {		
        ...
        if (!empty($postdata)){		
            ...
            if (!empty($res)) {						
                if (isset($res->status)) {	  
					$op['link'] = url('resetpwd-link/'.$res->token);
                    $op['msg']  = $res->msg;
                    $op['status'] = $this->statusCode = $res->status;											
                } elseif (isset($res->error)) {
                     $op['error'] = $res->error;
    ...
    {
        $op = $data = $usrdata = [];				
        $data['msg'] = 'The password reset link was invalid / expired (valid for 5 minutes), possibly because it has already been used. Please request a new password reset';		
		$data['token'] = '';
        $data['pwd_resetfrm'] = false;		
        $data['status'] = $this->config->get('httperr.UN_PROCESSABLE');		
        if (!empty($token) && strpos($token, '.'))
		{			
			$res = guzzle::getResponse('api/v1/user/resetpwd-link/'.$token, 'POST', [], []);			
			if (!empty($res)) {		
				if($res->status == $this->config->get('httperr.SUCCESS')){				
		...
		$data['rpfields'] = CommonNotifSettings::getHTMLValidation('ecom.reset_pwd');			
		return view('ecom.account.forgot_pwd', (array) $data);      
    }
