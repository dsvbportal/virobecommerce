...
class Member extends BaseModel
{
    ...
    public function member_list ($data = array(), $count = false)
    {
        ...
        $query = DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                ...
                ->join($this->config->get('tables.ACCOUNT_SETTINGS').' as ast', 'ast.account_id', ' = ', 'am.account_id')
                ->where('am.is_deleted', $this->config->get('constants.NOT_DELETED'))
        ...
        if (isset($search_text) && !empty($search_text))
        {
            if (isset($filterTerms) && !empty($filterTerms))
            {
                ...
                $search_field = ['UserName'=>'am.uname', 'FullName'=>'concat_ws(" ",ad.first_name,ad.last_name)', 'Email'=>'am.email', 'Mobile'=>'concat_ws(" ",lcu.phonecode,am.mobile)'];
            ...
            else
            {
                $query->where(function($wcond) use($search_text)
                {
                    $wcond->Where('am.uname', 'like', $search_text)
                            ->orwhere(DB::Raw('concat_ws(" ",ad.first_name,ad.last_name)'), 'like', $search_text)
        ...
        {
            $result = $query
                    ->selectRaw('am.account_id,am.signedup_on,am.uname,am.email,am.activated_on,am.status,am.block,am.block_login,ast.is_account_verified,concat_ws(" ",ad.first_name,ad.last_name) as fullname,concat_ws(" ",lcu.phonecode,am.mobile) as mobile,lcu.country')
    ...
    public function member_view ($uname)
    {
        $result = DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                ...
                ->selectRaw('am.signedup_on,am.uname,am.email,am.activated_on,am.status,am.block,ast.is_account_verified,ad.first_name,ad.last_name,concat_ws(" ",ad.first_name,ad.last_name) as fullname,ad.dob,ad.profile_image,concat_ws(" ",lcu.phonecode,am.mobile) as mobile,lcu.country,agl.gender,adm.formated_address as address')
                ->where('am.is_deleted', $this->config->get('constants.NOT_DELETED'))
                ->where('am.account_type_id', $this->config->get('constants.ACCOUNT_TYPE.USER'))
    ...
    public function member_edit ($uname)
    {
        return DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                        ...
                        ->join($this->config->get('tables.LOCATION_COUNTRIES').' as lcu', 'lcu.country_id', ' = ', 'adm.country_id')
                        ->selectRaw('am.uname,am.email,am.mobile,ad.first_name,ad.last_name,ad.gender,ad.dob,adm.flatno_street,adm.landmark,adm.address,adm.city_id,adm.district_id,adm.state_id,adm.country_id,adm.postcode')
                        ->where('am.is_deleted', $this->config->get('constants.NOT_DELETED'))
                        ->where('am.account_type_id', $this->config->get('constants.ACCOUNT_TYPE.USER'))
                        ->where('am.uname', $uname)
                        ->first();
    ...
    public function member_verify_status (array $data = array())
    {
        ...
        if (isset($result->account_id) && $result->account_id > 0)
        {
            if (isset($status) && $status == 1)
            {
                return DB::table($this->config->get('tables.ACCOUNT_SETTINGS'))
                                ->where('account_id', $result->account_id)
    ...
    public function member_pwd_change ($uname)
    {
        if (isset($uname) && !empty($uname))
        {
            $result = DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                    ->join($this->config->get('tables.ACCOUNT_DETAILS').' as ad', 'ad.account_id', ' = ', 'am.account_id')
                    ->selectRaw('am.uname,concat_ws(" ",ad.first_name,ad.last_name) as fullname')
                    ->where('am.is_deleted', $this->config->get('constants.NOT_DELETED'))
                    ->where('account_type_id', $this->config->get('constants.ACCOUNT_TYPE.USER'))
                    ->where('am.uname', $uname)
                    ->first();
    ...
    public function get_member_details (array $arr)
    {
        ...
        $qry = DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                ->leftjoin($this->config->get('tables.ACCOUNT_DETAILS').' as ad', 'ad.account_id', '=', 'am.account_id')
                ->leftjoin($this->config->get('tables.ACCOUNT_PREFERENCE').' as ast', 'ast.account_id', '=', 'am.account_id')
				->leftjoin($this->config->get('tables.CURRENCIES').' as cur', 'cur.currency_id', '=', 'ast.currency_id')
        ...
        if (isset($member) && !empty($member))
        {
            ...
            else if (is_numeric($member))
            {
				...
				$qry->where(function($wcond) use($member)
				{
					$wcond->Where('am.mobile', '=', $member)
							->orWhere('am.user_code', '=',$member);
				});
            }
            else
            {
        ...
        return $qry->selectRaw("am.uname,am.user_code,am.email,am.account_id,am.account_type_id,am.is_affiliate,am.mobile,CONCAT_WS(' ',ad.firstname,ad.lastname) as full_name,ast.country_id,ast.currency_id,cur.currency as currency_code")
                        ->first();
    }
