...
class Support extends BaseModel {
	...
	/* Faq List */
	public function get_faqs (array $arr = array()) 
    {   
        extract($arr);		
        $res = DB::table(config('tables.FAQS').' as f')
		        ->join(config('tables.FAQ_CATEGORIES').' as fc', function($subquery) use($account_type_id)
				{
					$subquery->on('fc.faq_category_id', ' = ', 'f.category_id')
					->where('fc.account_type_id', '=', $account_type_id);
				})	
				->join(config('tables.FAQ_LANG').' as fl', function($subquery)
				{
					$subquery->on('fl.faq_id', ' = ', 'f.id')
					->where('fl.lang_id', '=', config('app.locale_id'));
				})				
				->where('f.status', config('constants.ON'))
				->where('f.is_deleted', config('constants.OFF'));
        if (isset($faq_word) && !empty($faq_word))
        {
            $res->where(function($query) use ($faq_word)
            {
                $query->where('fl.questions', 'LIKE', '%'.$faq_word.'%')
                        ->orWhere('fl.answers', 'LIKE', '%'.$faq_word.'%');
            });
        }
        $res = $res->selectRaw('fl.questions,fl.answers')->get();		
	...
	public function get_anouncements ($doctype=null)
    {  
	    if(!is_null($doctype)){
		    $qry = DB::table(config('tables.AFF_DOCUMENT_SHARING'))
					->where('status', config('constants.ON'));
			if($doctype == config('constants.ON')){
				$qry->where('doc_type', $doctype);
			}else if($doctype == config('constants.OFF')) {
				$qry->where('doc_type', $doctype);
			}           
			$result = $qry->orderBy('created_date', 'DESC')
			              ->get(); 	
			              
			if(!empty($result)){
			    foreach($result as $res){				   
			        $res->doc_path = !empty($res->doc_path) ? ($doctype == config('constants.ON')) ? asset(config('constants.UPDATES_DOWNLOWD_VIEWPATH.WEB').$res->doc_path) : asset(config('constants.UPDATES_ANOUNCEMENT_VIEWPATH.WEB').$res->doc_path) :'';
			    }
			}
			return $result;
		}
