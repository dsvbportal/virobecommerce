...
class BaseModel extends Model
{
    ...
    public function __construct ()
    {
		...
		$this->sessionName = 'userdata';
		$this->commonstObj = new CommonSettings;		
		$this->commonObj = new CommonModel;		
		if((request()->is('affiliate/*') || request()->is('affjoin/*'))){
			if($this->session->has('userdata')){
				$this->userSess = (object)$this->session->get('userdata');
			}
			$this->sessionName = 'userdata';
		} 
		elseif(request()->is('channel-partner/*')){
			if($this->session->has('frdata')){
				$this->userSess = (object)$this->session->get('frdata');
			}
			$this->sessionName = 'frdata';
		}
    }
    ...
    {		  	
        ...
        $ngo_amt = (isset($ngo_amt) && !empty($ngo_amt)) ? $ngo_amt : 0;    		
        ...
        {		
            ...
            if ($status)
            {
                ...
                $debitted = DB::table($this->config->get('tables.ACCOUNT_TRANSACTION'))
                    ...
                    'current_balance'=>$bal,
                    'statementline_id'=>(isset($statementline_id))?$statementline_id:$this->config->get('stline.'.$transaction_for.'.DEBIT'),
					'current_balance'=>$bal,
                    'remark'=>isset($remark) ? $remark : addslashes(json_encode(['data'=>$debit_remark_data]))
                ]);
                if ($debitted)
                {
        ...
        {	
            ...
            { 
                ...
                $credited = DB::table($this->config->get('tables.ACCOUNT_TRANSACTION'))
                    ...
                    'current_balance'=>$bal,
                    'statementline_id'=>(isset($statementline_id))?$statementline_id:$this->config->get('stline.'.$transaction_for.'.CREDIT'),
                    'remark'=>isset($remark) ? $remark : addslashes(json_encode(['data'=>$credit_remark_data]))
                ]);				
   ...
   public function updateAccountTransaction_v1 (array $arr = array(), $debit_only = false, $credit_only = false, $trans_only = false, $deduct_chares_too = true)
    {		
        $supplier_id = 0;
        $store_id = 0;
        $mr_debit_trans_only = false;
        $mr_credit_trans_only = false;
        $debit_remark_data = [];
        $credit_remark_data = [];
        $debitted = $credited = false;
        $tax_amt = $handle_amt = 0;
        ...
        $pay_mode = isset($pay_mode) && !empty($pay_mode) ? $pay_mode : $this->config->get('constants.PAYMENT_MODES.CASH');
		
        $from_wallet_id = (isset($from_wallet_id) && !empty($from_wallet_id)) ? $from_wallet_id : $this->config->get('constants.CASHBACK_CREDIT_WALLET');
        ...
        if ($from_account_id != $this->config->get('constants.ACCOUNT.ADMIN_ID'))
        {
            if (isset($from_handle) && !empty($from_handle))
            {
                array_walk($from_handle, function(&$charge) use(&$handle_amt, $currency_id)
                {
                    $handle_amt += $charge['amt'];
                    $charge['currency_id'] = $currency_id;
                    $charge['debit_only'] = isset($charge['debit_only']) ? $charge['debit_only'] : false;
                    $charge['credit_only'] = isset($charge['credit_only']) ? $charge['credit_only'] : false;
                });
            }
            if (isset($from_taxes) && !empty($from_taxes))
            {
                array_walk($from_taxes, function(&$tax) use(&$tax_amt)
                {
                    $tax_amt += $tax['amt'];
                    $tax['debit_only'] = isset($tax['debit_only']) ? $tax['debit_only'] : false;
                    $tax['credit_only'] = isset($tax['credit_only']) ? $tax['credit_only'] : false;
                });
            }
        }
        if (!$credit_only)
        {			
            ...
            $tot_amt = $deduct_chares_too ? ($amt + $handle_amt + $tax_amt) : $amt;
			
            $bal = $this->updateBalance($from_account_id, $from_wallet_id, $currency_id, $trans_only ? 0 : $tot_amt, false);
            if ($trans_only || $bal)
            {
                $from_transaction_id = isset($from_transaction_id) && !empty($from_transaction_id) ? $from_transaction_id : $this->generateTransactionID();
                ...
                $debitted = DB::table($this->config->get('tables.ACCOUNT_TRANSACTION'))
                    ...
                    'transaction_id'=>$from_transaction_id,
                    'transaction_type'=>$this->config->get('constants.TRANSACTION_TYPE.DEBIT'),
                    'statementline_id'=>isset($statementline) ? $statementline : $this->config->get('stline.'.$transaction_for.'.DEBIT'),
                    'current_balance'=>$bal,
                    'remark'=>isset($remark) ? $remark : json_encode(['data'=>$debit_remark_data])
                ]);				
				
                if (!empty($debitted) && !empty($supplier_id) && ($mr_credit_trans_only == false))
                {
                    $merchant_trans['supplier_id'] = $supplier_id;
                    $merchant_trans['store_id'] = $store_id;
                    $merchant_trans['transaction_id'] = $debitted;
                    $merchant_trans['statementline'] = isset($statementline) ? $statementline : $this->config->get('stline.'.$transaction_for.'.DEBIT');
                    DB::table($this->config->get('tables.MERCHANT_CLIENT_TRANSACTION'))
                            ->insertGetID($merchant_trans);
                }
				
                 if ($debitted && $from_account_id != $this->config->get('constants.ACCOUNT.ADMIN_ID'))
                {
                    if (isset($from_taxes) && !empty($from_taxes) && $tax_amt != 0)
                    {
                        foreach ($from_taxes as $charge)
                        {
                            if(isset($supplier_id) && isset($store_id)){
								if (!$this->updateAccountTransaction(array(
											...
											'credit_remark_data'=>isset($charge['credit_remark_data']) ? $charge['credit_remark_data'] : [],
											'debit_remark_data'=>isset($charge['debit_remark_data']) ? $charge['debit_remark_data'] : []
												), false, true, true))
								{
									break;
								}
                    ...
                    if (isset($from_handle) && !empty($from_handle) && $handle_amt != 0)
                    {
        ...
        if (!$debit_only && ($credit_only || $debitted))
        {
            ...
            $currency_id = (isset($to_currency_id) && !empty($to_currency_id)) ? $to_currency_id : $currency_id;
            $amt = (isset($to_amt) && !empty($to_amt)) ? $to_amt : $amt;
            $bal = $this->updateBalance($to_account_id, $to_wallet_id, $currency_id, $trans_only ? 0 : $amt);
            if ($bal || $trans_only)
            {
                $to_transaction_id = isset($to_transaction_id) && !empty($to_transaction_id) ? $to_transaction_id : $this->generateTransactionID();
                $tax_amt = $handle_amt = 0;
                if ($to_account_id != $this->config->get('constants.ACCOUNT.ADMIN_ID'))
                {
                    ...
                    if (isset($to_taxes) && !empty($to_taxes))
                    {
                        array_walk($to_taxes, function(&$tax) use(&$tax_amt)
                        {
                            $tax_amt += $tax['amt'];
                            $tax['debit_only'] = isset($tax['debit_only']) ? $tax['debit_only'] : false;
                            $tax['credit_only'] = isset($tax['credit_only']) ? $tax['credit_only'] : false;
                 ...
                 $credited = DB::table($this->config->get('tables.ACCOUNT_TRANSACTION'))
                    ...
                    'transaction_type'=>$this->config->get('constants.TRANSACTION_TYPE.CREDIT'),
                    'current_balance'=>$bal,
                    'statementline_id'=>isset($statementline) ? $statementline : $this->config->get('stline.'.$transaction_for.'.CREDIT'),
                    'remark'=>isset($remark) ? $remark : json_encode(['data'=>$credit_remark_data])
                ]); 
                if (!empty($credited) && !empty($supplier_id) && ($mr_debit_trans_only == false))
                {
                    $merchant_trans['supplier_id'] = $supplier_id;
                    $merchant_trans['store_id'] = $store_id;
                    $merchant_trans['transaction_id'] = $credited;
                    $merchant_trans['statementline'] = isset($statementline) ? $statementline : $this->config->get('stline.'.$transaction_for.'.CREDIT');
                    DB::table($this->config->get('tables.SUPPLIER_TRANSCATION'))
                            ->insertGetID($merchant_trans);
                }
                if ($credited && $to_account_id != $this->config->get('constants.ACCOUNT.ADMIN_ID'))
                {
                    if (isset($to_taxes) && !empty($to_taxes) && $tax_amt != 0)
                    {
                        foreach ($to_taxes as $charge)
                        {
                            if (!$this->updateAccountTransaction(array(
                                        'supplier_id'=>$supplier_id,
                                        'store_id'=>$store_id,
                    ...
                    if (isset($to_handle) && !empty($to_handle) && $handle_amt != 0)
                    {
                        foreach ($to_handle as $charge)
                        {
                            if (!$this->updateAccountTransaction(array(
                                        'supplier_id'=>$supplier_id,
                                        'store_id'=>$store_id,
    ...
    {		
		...
		if ($increment || (!$increment && DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                        ...
                        ->where(function($b) use($tax_less_amount)
                        {
							...
							->orWhere('am.is_affiliate', $this->config->get('constants.ON'));
                        }) 
                        ->exists()))
        {
    ...
    public function updateBalance_v1 ($account_id, $wallet_id, $currency_id, $amount, $increment = true)
    {			
        $balance = DB::table($this->config->get('tables.ACCOUNT_BALANCE'))
                ->where('account_id', $account_id)
                ->where('currency_id', $currency_id)
        ...
        if ($balance)
        {
            if ($amount > 0)
            {
                ...
                else if ($balance->current_balance >= $amount || DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                                ->join($this->config->get('tables.ACCOUNT_TYPES').' as sr', 'sr.id', '=', 'am.account_type_id')
								->where('am.account_id','=', $account_id)
                                ->where(function($sq){									
									$sq->where('sr.allow_minus_balance', $this->config->get('constants.ON'))
									->orWhere('am.is_affiliate', $this->config->get('constants.ON'));
								})								
                                ->exists())
                {
                ...
                DB::table($this->config->get('tables.ACCOUNT_BALANCE'))
                        ...
                        ->where('currency_id', $currency_id)
                        ->where('wallet_id', $wallet_id)
                        ->update(['tot_credit'=>$balance->tot_credit, 'tot_debit'=>$balance->tot_debit, 'current_balance'=>$current_balance, 'updated_on'=>getGTZ()]);
        ...
        else
        {
            ...
            {
                DB::table($this->config->get('tables.ACCOUNT_BALANCE'))
                        ->insert(['account_id'=>$account_id, 'currency_id'=>$currency_id, 'wallet_id'=>$wallet_id, 'tot_credit'=>$amount, 'current_balance'=>$amount, 'updated_on'=>getGTZ()]);
                return $amount;
            }
            else if (DB::table($this->config->get('tables.ACCOUNT_MST').' as am')
                            ->join($this->config->get('tables.ACCOUNT_TYPES').' as sr', 'sr.id', '=', 'am.account_type_id')
                            ->where('am.account_id', $account_id)
                            ->where('sr.allow_minus_balance', $this->config->get('constants.ON'))
                            ->exists())
            {
                $amount = (-1) * $amount;
                DB::table($this->config->get('tables.ACCOUNT_BALANCE'))
...
//$usrRecCnt = DB::table($this->config->get('tables.ACCOUNT_SETTINGS'))->selectRaw('MAX(LENGTH(referral_code)) as ref_lenexist'),DB::Raw('MAX(account_id) as idLength')->first();
    ...
    {		
		...
		$tax = DB::table($this->config->get('tables.STATEMENT_LINE').' as st')
                ...
                ->selectRaw('t.tax_id,tc.tax_class_id,CONCAT_WS(\'-\',tc.tax_class,t.tax) as tax,t.tax_value,t.settings,t.currency_id');		
        ...
        foreach ($taxes as &$tax)
        {
            ...
            if (!empty($tax->settings))
            {
				...
				$a = true;			
                if (isset($tax->settings->earnings) && $a)
                {                    
					//echo 'init';
		...
		if(!empty($taxes)){			
			foreach($taxes as $tax){
				$taxArr[] = ['tax_id'=>$tax->tax_id,'tax_title'=>$tax->tax,'tax_val'=>$tax->tax_value,'tax_amt'=>$tax->tax_amt];
			}
			$tax_json = addslashes(json_encode($taxArr));
        ...
        //return [$total_tax_amount, array_filter($taxes),$tax_class_id,$tot_tax_perc,$tax_json];
		return $op;
    } 
