...
class CustomValidationRulesServiceProvider extends ServiceProvider
{
    ...
    public function boot ()
    {
		...
		Validator::extend('panumber', function($attribute, $value, $parameters, $validator)
        {
        ...
        {            
			return preg_match('/^[a-z\sA-Z]+$/', $value);
        });
		...
		Validator::extend('idOremail', function($attribute, $value, $parameters, $validator)
        {            
			if(strpos($value,'@')>-1){
		...
		Validator::extend('pwdcheck', function($attribute, $value, $parameters, $validator)
        {
			$data = $validator->getData();
			print_R($parameters);
			$query = \DB::table(array_shift($parameters));            
            $column = array_shift($parameters);			
            $query->where($column, md5($value));
		...
		Validator::extend('tpin_match', function($attribute, $value, $parameters, $validator)
        {
			$data = $validator->getData();
			$query = \DB::table(array_shift($parameters));            
            $column = array_shift($parameters);			
