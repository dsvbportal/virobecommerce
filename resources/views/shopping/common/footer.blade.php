<footer class="off-white-bg2 pt-50 bdr-top pt-sm-55">
        ...
        <!-- Footer Area End Here -->
		 <div class="modal fade" id="login-model">
               ...
               <form method="post" id="loginfrm" action="{{route('ecom.checklogin')}}">                                    
                              <h2 class="title">Login</h2>
                              <div class="input-container">
                                <input {!!build_attribute($validiate['lfields']['username']['attr'])!!} id="username" name="username">
                                <label for="#{label}">Email address/Mobile No</label>
                              </div>
                              <div class="input-container">
                                <input {!!build_attribute($validiate['lfields']['password']['attr'])!!} type="password" id="password" name="password">
                     ...
                     <form  method="post" id="signupfrm" action="{{route('ecom.sign_up_save')}}" >
                                <h2 class="title">Signup</h2>
                                <div class="input-container">
                                  <input {!!build_attribute($validiate['regfields']['full_name']['attr'])!!} type="text" id="full_name" name="full_name">
                                  <label for="#{label}">FullName</label>
                                </div>
                               
                                <div class="input-container">
                                @foreach($validiate['countries'] as $key=>$con)
                                @if($con->iso2==$validiate['ip_country'])
                               <!--  <span  class="input-group-addon"><img src="{{$con->img_url}}" id="f_img"><label id='phonecode'>{{$con->phonecode}}</label></span> -->
                                ...
                                <div class="input-container">
                                  <select {!!build_attribute($validiate['regfields']['country']['attr'])!!} name="country" id="country" placeholder="Select Your Country">
                                    @foreach($validiate['countries'] as $key=>$con)
                                      @if($con->iso2==$validiate['ip_country'])
                                      <option value="{{$con->country_id}}" selected="selected" data_url="{{$con->img_url}}" data_code="{{$con->phonecode}}">{{$con->country_name}}
                                     </option>
                                    @else
                                     <option value="{{$con->country_id}}" " data_url="{{$con->img_url}}" data_code="{{$con->phonecode}}">{{$con->country_name}}
                                   </option>
                                   @endif
                                   @endforeach                                                          
                                  </select>
                                ...
                                <div class="input-container">
                                  <input {!!build_attribute($validiate['regfields']['email']['attr'])!!} type="text"  id="email" name="email">
                                ...
                                <div class="txt1">
                                  <input {!!build_attribute($validiate['regfields']['temsandcondition']['attr'])!!}  type="checkbox" id="temsandcondition" name="temsandcondition" placeholder="" data-err-msg-to="#err_msg_td"> I want to receive exclusive offers and promotions from virob.
               ...
               <form  method="post"  id="varification"  class="form-horizontal"  action="{{route('ecom.sign_up_varification')}}" style="display:none">                <h6>Enter OTP</h6>
                                ...
                                <div class="input-container">
                                  ...
                                  <label for="#{label}">Enter OTP</label>                           
                                   ...
                                   <p><a class="link" href="#" ctr_url="{{route('ecom.sign_up_resend_otp')}}" id="resendotp" >Resend OTP</a></p>
