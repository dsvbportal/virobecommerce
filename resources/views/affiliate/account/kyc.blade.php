					...
					<div class="box-body">
                        ...
                        <div class="tab-pane active" id="Tinpancard">
                                ...
                                <form action="{{route('aff.profile.kyc_upload')}}" class="form" method="post" enctype="multipart/form-data" id="document_upload">
									{!! csrf_field() !!}    
									@if(isset($tax_proof) && !empty($tax_proof))
								        <div class="form-group">
											<select name="document_types" id="document_types" class="form-control">
												<option value="">-- select --</option>				
												@foreach($tax_proof as $docitem)
													<option value="{{$docitem->document_type_id}}">{{$docitem->type}}</option>
												@endforeach
											</select>
								        </div>
								        <!--div class="radio">
											<label for="address_types'.$docitem->document_type_id.'">
												<input type="radio" name="document_types" value="'.$docitem->document_type_id.'" id="address_types'.$docitem->document_type_id.'"><b>'.$docitem->doctype_name.'</b>
											</label>
										</div-->									
									@endif
									<div class="form-group">
										<input type="file" name="verify_file" id="document_upload_file" data-formatallowd="gif|jpg|jpeg|png|pdf" data-error="Please select valid format(*.gif, *.jpg, *.jpeg,*.pdf, *.png)" class="file_upload">
									</div>
									<div class="form-group">
                                ...
                                <div class="box-footer">
                                    <button type="button" class="btn btn-primary pull-right"  name="submit_verification" id="document_upload_file_btn" data-form="document_upload">{{trans('affiliate/kyc.submit_btn')}}</button>
                                </div>
                                <!-- box-footer -->
                        ...
                        <div class="tab-pane" id="IDProof">
                            <div class="page-heading">
                                ...
                                <form action="{{route('aff.profile.kyc_upload')}}" method="post" enctype="multipart/form-data" id="photo_id">
									...
									@foreach ($document_types as $docitem)
								       <div class="radio"><label for="address_types'.$docitem->document_type_id.'"><input type="radio" name="document_types" value="'.$docitem->document_type_id.'" id="address_types'.$docitem->document_type_id.'"><b>'.$docitem->type.'</b></label></div>
									@endforeach
									@endif
									...
									if(isset($document_types) && !empty($document_types)){
                                    foreach ($document_types as $docitem){
                                        echo '<div class="radio"><label for="address_types'.$docitem->document_type_id.'"><input type="radio" name="document_types" value="'.$docitem->document_type_id.'" id="address_types'.$docitem->document_type_id.'"><b>'.$docitem->type.'</b></label></div>';
                                    }
									}
                                    ? -->
                            ...
                            <!-- box-footer -->
                            <div class="box-footer">
                            	<button type="button" class="btn btn-primary pull-right"  name="submit_verification" id="photo_id_file_btn" data-form="photo_id">{{trans('affiliate/kyc.submit_btn')}}</button>
                         	</div>
                            <!-- box-footer -->
                        </div>
                        <div class="tab-pane" id="addproof">
                            <div class="page-heading">
                                ...
                                <form action="{{route('aff.profile.kyc_upload')}}" method="post" enctype="multipart/form-data" id="address_id">
									...
									@foreach ($document_types1 as $docitem)
								       <div class="radio"><label for="address_types'.$docitem->document_type_id.'" class="control-label"><input type="radio" name="document_types" value="'.$docitem->document_type_id.'" id="address_types'.$docitem->document_type_id.'"><b>'.$docitem->type.'</b></label></div>
									@endforeach
									@endif
									...
									if(isset($document_types1) && !empty($document_types1)){
                                    foreach ($document_types1 as $docitem) {
                                        echo '<div class="radio"><label for="address_types'.$docitem->document_type_id.'" class="control-label"><input type="radio" name="document_types" value="'.$docitem->document_type_id.'" id="address_types'.$docitem->document_type_id.'"><b>'.$docitem->type.'</b></label></div>';
                                    }
									}
                                    ?-->
									@if(isset($address_proof) && !empty($address_proof))
								        <div class="form-group">
											<select name="document_types" id="document_types" class="form-control">
												<option value="">-- select --</option>				
												@foreach($address_proof as $docitem)
													<option value="{{$docitem->document_type_id}}">{{$docitem->type}}</option>
												@endforeach
											</select>
								        </div>								        									
                            ...
                            <!-- box-footer -->
                            <div class="box-footer">
                            	<button type="button" class="btn btn-primary pull-right"  name="submit_verification" id="address_id_file_btn" data-form="address_id">{{trans('affiliate/kyc.submit_btn')}}</button>
                         	</div>
                            <!-- box-footer -->
                        </div>
                    </div>
...
<script src="{{asset('affiliate/validate/lang/user-verification')}}" type="text/javascript" charset="utf-8"></script>
