	...
	<div class="signup-right">
		...
		<div class="small-12 small-centered columns regbox signup">
			...
			<form id="check_acform" action="{{route('aff.signup.acverify')}}" method="post">
				<div class="row">
					<div class="form-group">
						...
						<div class="absFld">
						<input type="text" name="login_id" id="login_id"  placeholder="Email" value="" />						 
						<select name="phonecode" id="phonecode" class="form-control">
							<option value="">Select</option>
							@if(!empty($countries))
							@foreach ($countries as $country_val)
							<option value="{{$country_val->iso2}}" data-id="{{$country_val->country_id}}" {{(isset($ip_country) && $ip_country==$country_val->iso2)? "selected=selected":''}}>{{$country_val->phonecode}}</option>
							@endforeach
							@endif
						</select>
						</div>
			...
			<form id="extSignupFrm" action="{{route('aff.signup.acupgrade')}}" method="post" style="display:none">
					...
					<div class="form-group">
						<div class="small-6 medium-6 columns">
							<label>{{trans('affiliate/signup.gardians')}}
							 <input type="text" name="gardian" id="gardian"  placeholder="Father's/Husband's Name" value=""/>
							 </label>
				...
				<div class="row">
					<div class="small-6 medium-6 columns">
						<label>{{trans('affiliate/signup.mobile_no')}}<br>
							<div class="absFld">
								<select name="country" id="country" class="form-control">
									<option value="">Select</option>
									@if(!empty($countries))
									@foreach ($countries as $country_val)
									<option value="{{$country_val->iso2}}" data-id="{{$country_val->country_id}}">{{$country_val->phonecode}}</option>
									@endforeach
									@endif
								</select>
				...
				<div class="row">
					<div class="col-sm-12 columns">
						<input type="submit" id="submit_button" class="button large expand" value="CREATE ACCOUNT">
						<p>
			...
			<div id="signup_section" style="display:none">
				<form id="signupFrm" action="{{route('aff.signup.save')}}" method="post">
					...
					<div class="row">
						<div class="col-sm-12 columns terms-text">
							<input type="submit" id="submit_button" class="button large expand" value="CREATE ACCOUNT">
							<p>
								<?php echo trans('affiliate/signup.privacy_txt',['site_name'=>$pagesettings->site_name,'terms_link'=>url('terms-and-conditions'),'privacy_link'=>url('privacy-policy'),'cookie_privacy_link'=>url('privacy-policy')])?>
						</div>
					</div>
				</form>
			</div>
...
<div class="regconfirm" style="display:none">
	<div class="small-10 large-8 medium-8  small-centered columns signup text-center">
		<div class="row">
			...
			<p>We've sent an verification email to completed your registration. Please check your inbox.</p>
		</div>
	</div>
</div>
