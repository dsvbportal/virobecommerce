     <form method="post" action="{{route('aff.wallet.fund_transfer_confirm')}}"class="form-horizontal form-bordered" id="fundtransferform" autocomplete="off" onsubmit="return false;">
		...
		<div class="form-group">   
			   ...
			   <div class="col-lg-8 form_field">
			   <label class="control-label"  for="user_avail_bal">{{number_format($wallet_balance[0]->current_balance,'2').' '.$wallet_balance[0]->currency_code}}</label>
		...
		<div class="form-group">
				...
				<input type="text" name="to_account" id="to_account" class="form-control" value="{{ (isset($rec_email) &&!empty($to_account))? $to_account : ''}}" />
				
				<div class="input-group-btn">
					<button type="button" value="search" class="btn btn-md btn-warning btn-flat" onclick="user_check();"> 
			...
			<div class="form-group hidefld2">
				<label class="col-lg-4 col-sm-2 control-label" for="amount">{{\trans('affiliate/wallet/fundtransfer.amount')}} *</label>
				 ...
				 <span class="input-group-btn">
						<button class="btn btn-default" type="button">{{$wallet_balance[0]->currency_code}}</button>
				  </span>
