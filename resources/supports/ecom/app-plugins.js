$.fn.extend({	
    loadMenus: function (data) {
        ...
        _this.loadData = function () {			
            if (_this.menus == {}) {
                $.ajax({
                    ...
                    url: 'http://localhost/dsvb_affiliate/get-menus',
                    success: function (op) {
        ...
        _this.print = function () {			
            _this.find('li#header_catalogue').empty().append(_this.addHeaderCatalogueMenu(_this.menus.header_catalogue));
            _this.find('ul#header_primary').empty().append(_this.addHeaderHeaderPrimaryMenu(_this.menus.header_primary));
								...
								function () {
									...
									$.each(menu.group, function (kg, g) {
										arr.push($('<li>', {class: 'block-container col-sm-3'}).append(
											...
											/* $.each(g, function (ck, c) {
												console.log(c.links); 	
												if (c.links !== undefined) {
													$.each(c.links, function (kl, l) {
														gr.push($('<li>', {class: 'link_container'}).append($('<a>', {href: l.url}).append(l.title)));
													});
												}
											}); */
        ...
        _this.addHeaderCatalogueMenu = function (menus) {
            ...
            if (menus !== undefined && menus.length) {
                $.each(menus, function (k, menu) {
                    if (menu.group !== undefined) {
                        m.push($('<li>').append([$('<a>', {class: 'parent', href: menu.url}).append([$('<img>', {clas: 'icon-menu'}), menu.title]), $('<div>', {class: 'vertical-dropdown-menu'}).append($('<div>', {class: 'vertical-groups col-sm-12'}).append($('<div>', {class: 'mega-group col-sm-4'}).append(function () {
                                var arr = [];
                                $.each(menu.group, function (kg, g) {
