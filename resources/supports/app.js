            ...
            if (CURFORM != undefined && CURFORM !== null && data !== undefined &&  data.error !== undefined && data.error !== null && xhr.status === 400) {
			    ...
			    if (CURFORM.data('errmsg-fld')==undefined) {				
					...
					if(data.msgclass!==undefined){
						CURFORM.before('<div class="alert alert-'+data.msgclass+' alert-err"><a href="#" class="close" area-label="close" data-dismiss="alert">&times;</a>'+ data.msg + '</div>')
					}
					else {
...
function isNumberKey(evt) {
    ...
    if (charCode > 31 && (charCode < 48 || charCode > 57 || charCode == 46 ) && !(charCode >= 96 && charCode <= 108)) {
        return false;
    }
    return true;
}
...
function isNumberKeydot(evt) {	
    ...
    if (charCode != 46 && charCode > 31 && (charCode > 57 || charCode < 48 || charCode == 46) && charCode !=110 && !(charCode >= 96 && charCode <= 108)) {		
	alert(charCode)	;
        return false;
    }
    return true;
...
function RestrictNumericDot(evt) {	
    if (event.keyCode == 110 || event.keyCode == 190) {
		val  = evt.target.value;
		if(val.split('.').length>2){
			evt.target.value = val.split('.')[0]+'.'+val.split('.')[1];
		}		
    }
}
