                        ...
                        if (confirm('Are you sure, you wants to proceed?')) {
                            ...
                            $.ajax({
                                ...
                                success: function (data)
                                {
									...
									$('#create_user').before('<div class="alert alert-success"><a href="#" class="close" area-label="close" data-dismiss="alert">&times;</a>'+ data.msg + '</div>');
									$('.alert').fadeOut(5000);	
                                },
                                error: function () {
                                    $('#save_user').removeAttr('disabled');
  ...
  $('#email').on('change', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status == 200) {
                    EmailChecked = true;
                   /* $(".email_err").after('<div for="email" class="text-success email-success">' + data.msg + '</div>');*/
					$(".email_err").after('<div for="email" class="text-success email-err">' + data.msg + '</div>');
                } 
                else {
                    EmailChecked = false;
					 $(".email_err").after('<div for="email" class="help-block email-err">' + data.msg + '</div>');
                   /* $(".email_err").after('<div for="email" class="help-block email-err">' + data.msg + '</div>');*/
                }
            },
            error: function () {
            ...
            success: function (data) {
                ...
                if (data.status == 200) {
                 ...
                 /*   CurEle.after('<div for="mobile" class="text-success mobile-err">' + data.msg + '</div>'); */
					 $(".mobile_err").after('<div for="mobile" class="text-success mobile-err">' + data.msg + '</div>');
                } 
                else {
                    MobileChecked = false;
                   /* CurEle.after('<div for="mobile" class="help-block mobile-err">' + data.msg + '</div>'); */
				    $(".mobile_err").after('<div for="mobile" class="help-block mobile-err">' + data.msg + '</div>');
                }
            },
            error: function () {
                alert('Something went wrong');
...
$("#country_id").change(function () {
	...
	$("#phonecode").text($('option:selected',$(this)).data('phonecode'));	   
});
/*
