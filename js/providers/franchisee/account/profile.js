...
$(document).ready(function () {
    ...
    PIF.on('submit', function (e) {
		...
		$.ajax({
            ...
            success: function (data) {						
				...
				$("#franchisee_logo").attr("src", data.logo_path); 
				$("#frans_logo").attr("src", data.logo_path); 
            },
			error: function (event, xhr, settings) {
				var op = event.responseJSON;
				$('div.alert').remove();
	...
	$(document).on('click','#changeEmailBtn', function (e) {
        ...
        $.ajax({
			...
			success: function (data) {							
				/* if(data.status == 200){
					$('#profile_editfrm').before("<div class='alert alert-success'>"+data.msg+"</div>");				
				} */	
			}
		});
    });
    ...
    VFO.validate({
		...
		messages : $email_validate_otp_message, 
		submitHandler: function (form, event) {
            event.preventDefault();
			CURFORM =VFO;
			if (VFO.valid()) {
 ...
 var CMF = $('#change-mobile-form');
	...
	$(document).on('click','#verifyMobile_Resend_otp', function (e) {
		e.preventDefault();
		CURFORM = $("#verifymobile_otpform");
        $.ajax({
			url: $(this).data('url'),
